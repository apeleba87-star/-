...
export default function DemandGuestLoginCta({ shareTeaser = false }: Props) {
      ...
      <div className="mx-auto flex max-w-lg flex-col items-stretch gap-2">
        {shareTeaser ? (
          <p className="text-center text-xs leading-snug text-slate-600">
            ...
            <br />
            <span className="font-medium text-slate-800">로그인하면 검색·그래프까지 전체 데이터</span>를 볼 수
            있습니다.
          </p>
        ) : (
          <p className="text-center text-xs leading-snug text-slate-600">
        ...
        >
          {shareTeaser ? "로그인하고 전체 데이터 보기" : "로그인하고 수치 보기"}
        </Link>
