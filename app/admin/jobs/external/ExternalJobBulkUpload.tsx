...
export default function ExternalJobBulkUpload() {
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    try {
      ...
      fd.set("file", file);
      const res = await fetch("/api/admin/jobs/bulk-upload", {
        method: "POST",
        body: fd,
      });
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-600">
        템플릿 「청소업종기준표」는 구인 등록 화면의 <strong>작업 종류 태그</strong>(유리청소, 입주청소 등)와 같은{" "}
        <strong>소분류</strong> 목록입니다. 「업로드양식」의{" "}
        <code className="rounded bg-slate-100 px-1 text-xs">category_sub_id</code> 열에 해당 UUID를
        복사하세요. (예전 상위 3개만 보이던 것은 상위 카테고리만 조회했기 때문입니다.)
      </p>
