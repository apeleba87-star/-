...
import { getActiveEstimateAnalyzeBelowAd } from "@/lib/ads";
import CleaningEstimateCalculator from "@/components/estimate/CleaningEstimateCalculator";

export const revalidate = 60;

export default async function EstimatePage() {
  const [config, analyzeBelowAd] = await Promise.all([
    getEstimateConfig(),
    getActiveEstimateAnalyzeBelowAd(),
  ]);
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <CleaningEstimateCalculator config={config} analyzeBelowAd={analyzeBelowAd} />
    </div>
  );
}
